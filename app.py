...
detection_logs = []       # Stores dicts: {"name": "...", "time": "..."}
last_logged_time = {}     # Prevents duplicate entries every frame (5s cooldown)

camera = cv2.VideoCapture(0)


def generate_frames():
    global current_mode, target_name, sample_count, recognizer, label_map, detection_logs, last_logged_time
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            # MODE 2: Live Face Recognition & Timestamp Logging
                ...
                # Log Name & Time (with 5-second cooldown per person)
                now_sec = time.time()
...
def stop_system():
    ...
    target_name = ''
    sample_count = 0
    return jsonify({"status": "success", "message": "System stopped. Returned to idle."})


@app.route('/api/captures', methods=['GET'])
def get_captures():
    """Returns the total captures count and recent detection logs."""
    return jsonify({
        "status": "success",
        "count": len(detection_logs),
        "logs": detection_logs[:15]  # Returns the 15 most recent captures
    })
